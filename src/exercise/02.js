...
function Greeting({initialName = ''}) {
  ...
  //LAZY INIITIALIZER: incializador "preguiçoso"
  //Quando o useState recebe uma função em vez de um valor como estado inicial,
  //essa função é executada apenas durante a fase mount do componente, sem se repetir na fase update
  ...
  function handleChange(event) {
    ...
    React.useEffect(()=> {
        //O valor do localStorage será atulalizado após a atualização do componente
        window.localStorage.setItem('name', name)
        window.localStorage.setItem('nameUC', nameUC)
        setCount(count + 1)
    }, [name, nameUC]) //[] é a lista de depêndencias, ou seja, esse useEffect é para ser chamado apenas
    //quando a variável name sofrer alteração
